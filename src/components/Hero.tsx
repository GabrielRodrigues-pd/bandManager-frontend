...
export default function Hero() {
  return (
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden bg-background">
      ...
      <Orb className="w-[600px] h-[600px] top-[-10%] left-[-10%]" color="bg-vibrant-violet" />
      <Orb className="w-[500px] h-[500px] bottom-[10%] right-[-5%]" color="bg-vibrant-cyan" />
      <Orb className="w-[400px] h-[400px] top-[20%] right-[10%]" color="bg-vibrant-pink" />
      ...
      <div className="container mx-auto px-6 relative z-10 py-32 md:py-48">
        <div className="flex flex-col items-center text-center">
          ...
          >
            ...
            <span className="block">Way <span className="text-vibrant-glow italic">You Play</span></span>
          ...
          >
            A high-performance command center for bands. <br className="hidden md:block" />
          ...
          >
            {[
              { icon: Music, label: "Setlist Hub", desc: "Craft seamless transitions", color: "text-vibrant-violet" },
              { icon: Users, label: "Live Sync", desc: "Band-wide real-time state", color: "text-vibrant-cyan" },
              { icon: Calendar, label: "Gig Pulse", desc: "Active tour management", color: "text-vibrant-pink" },
            ].map((feature, i) => (
              <div key={i} className="glass-card p-10 rounded-[2.5rem] flex flex-col items-center group hover:-translate-y-2 transition-all border-white/5 relative overflow-hidden">
                {/* Internal Card Glow */}
                <div className={`absolute top-0 right-0 w-24 h-24 blur-[60px] opacity-20 group-hover:opacity-40 transition-opacity ${feature.color.replace('text', 'bg')}`} />
                
                <div className={`w-16 h-16 rounded-2xl bg-white/5 dark:bg-black/20 flex items-center justify-center mb-8 border border-white/10 group-hover:scale-110 group-hover:rotate-6 transition-all`}>
                  <feature.icon className={`w-8 h-8 ${feature.color}`} />
                </div>
                <h3 className="font-bold text-2xl mb-3 tracking-tight">{feature.label}</h3>
